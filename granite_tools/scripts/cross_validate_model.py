...
if __name__ == "__main__":
    ...
    x0 = get_initial_params_and_bounds()
    ...
    for trig_scores_train, trig_scores_valid in iter_training_validation_sets(
        ...
        scoresets_train = TrigramScoreSets.from_trigram_scores(trig_scores_train, hands)
        scoresets_valid = TrigramScoreSets.from_trigram_scores(trig_scores_valid, hands)
        func_train = get_trigram_params_error_fun(scoresets_train, hands, bigram_scores)
        func_valid = get_trigram_params_error_fun(scoresets_valid, hands, bigram_scores)

        params = optimize_parameters(func_train, x0, hands.config)
